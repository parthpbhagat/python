...
class Create_a_array:
    ...
    def median_array(self):
        try:
          ...
          print("median value is: ",self.median)
        except ValueError:
            print("Input Error: Please enter valid integers.")
        except Exception as e:
            print(f"Unexpected error: {e}")
    ...
    def create_2D_array(self):
        try:
            ...
            self.element = input(
                f"Enter {self.total_elements} elements of the array, sepreted by space: "
            )
            self.elements_list = list(map(int, self.element.split()))
            self.origneal_array = np.array(self.elements_list).reshape(
                self.row, self.coloumns
            )
            print("2D array created successfully")
            print("2D array: ", self.origneal_array)
        except ValueError:
            print("Input Error: Please enter valid integers.")
        except Exception as e:
            print(f"Unexpected error: {e}")
    ...
    def addition_array(self):
        try:
            self.element = input(
                f"enter the same size array ({self.total_elements} elements sepreted by space): "
            )
            self.elements_list = list(map(int, self.element.split()))
            self.second_array = np.array([self.elements_list]).reshape(
                self.row, self.coloumns
            )
    ...
    def subtraction_array(self):
        try:
            self.element = input(
                f"Enter the same size ({self.total_elements} elements sepreded bi space): "
            )
            self.elements_list = list(map(int, self.element.split()))
            self.second_array = np.array([self.elements_list]).reshape(
                self.row, self.coloumns
    ...
    def multiplication_array(self):
        try:
            self.element = input(
                f"Enter the same size of array ({self.total_elements} Elements sepreted by space): "
            )
            self.elements_list = list(map(int, self.element.split()))
            self.second_array = np.array([self.elements_list]).reshape(
                self.row, self.coloumns
    ...
    def combine_array_verticaly(self):
        try:
            self.element = input(
                f"enter the anather array to combine ({self.total_elements} seprated by space): "
            )
            self.elements_list = list(map(int, self.element.split()))
            self.second_array = np.array([self.elements_list]).reshape(
                self.row, self.coloumns
            ...
            self.combine = np.concatenate(
                (self.origneal_array, self.second_array), axis=0
            )
    ...
    def combine_array_hirizentaly(self):
        try:
            ...
            self.combine = np.concatenate(
                (self.origneal_array, self.second_array), axis=1
            )
    ...
    def search_the_array(self):
        try: 
          self.filter_array = np.array(self.origneal_array)
          print(self.filter_array)
          num = int(input("witch elements you are find: "))
          self.indicates = np.where(self.filter_array == num)
          print(self.indicates)
        except ValueError:
            print("Input Error: Please enter valid integers.")
        except Exception as e:
            print(f"Unexpected error: {e}")
    ...
    def filter_the_array(self):
        try: 
            self.filter_array  = np.array([self.array])
            self.filtered_array = self.filter_array[self.filter_array % 2 == 0]
            print("filtter even number: ",self.filtered_array)
        except ValueError:
            print("Input Error: Please enter valid integers.")
        except Exception as e:
            print(f"Unexpected error: {e}")
